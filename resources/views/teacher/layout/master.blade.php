            ...
            <div class="row mt-5">
                <div class="col-12">
                    <div class="alert alert-info font-weight-bold text-center" style="margin-top:2.4rem">
                        You Account is Active, You have {{ \Auth::user()->no_of_bids }} bids to get a job. Purchase a subscription now. <a href="{{ route('pricing') }}">Click here</a>
                    </div>
                </div>
            </div>
            @elseif(!\Auth::user()->is_subscribed && \Auth::user()->no_of_bids == 0)
                <div class="col-12">
                    <div class="alert alert-warning font-weight-bold text-center" style="margin-top:3.8rem">
                        Your free bids is complete, Purchase our subscription to get a job. <a href="{{ route('pricing') }}">Click here</a>
                    </div>
                </div>
            @elseif(\Auth::user()->is_subscribed && \Auth::user()->no_of_bids == 0)
                <div class="col-12">
                    <div class="alert alert-info font-weight-bold text-center" style="margin-top:3.8rem">
                        Your bids limit reached, Purchase subscription again to get more jobs. <a href="{{ route('pricing') }}">Click here</a>
                    </div>
                </div>
            @endif
           @yield('content')
